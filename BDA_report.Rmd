...
WORK <- c("1. 搜集有关“幸福感”的背景资料，包含幸福悖论、宗教、性别认同、住房等相关方面的背景  2. 主笔报告团队概况部分  3. 整理GitHub相关学习资料  4. 可视化分析教育、地域、社会认知等方面变量与主观幸福感的关系  5. 建模分析","1. 搜集有关“幸福感”的背景资料，包含政策、世界幸福感排名等相关资料  2. 主笔数据预处理部分  3. 可视化宗教信仰、生活方式、家庭状况等方面变量与主观幸福感的关系  4. 建模分析","1. 搜集有关“幸福感”的背景资料，包含政治、经济等方面的背景 2. 主笔数据来源以及问题背景部分  3. 可视化身心健康、工作就业等变量与主观幸福感的关系  4. 建模分析")
knitr::kable(
  data.frame(NAME,STUDENT_NUMBER,WORK),
...
```{r}
data_index<-read.csv("C:/Users/ZYJ/Desktop/happinessdata/happiness_index_1.csv")
data_index
```
...
#根据省份分组查看各省份幸福感均值
happiness_meanp <- summarise(group_by(data,province),meanp=mean(as.numeric(happiness)))
...
```
-  由表可知，广西壮族自治区的幸福感均值最低为3.623188，河北省幸福感均值最高为4.107143，但总体幸福感相差不大。
...
happiness_meane <- summarise(group_by(data,edu),meane=mean(as.numeric(happiness)))
...
-  由图分析，幸福感均值最大致值出现在教育程度为13（研究生及以上）处，但除去-8（未回答），其余教育程度的幸福感均值都在3.5以上，相差不大。因此，教育程度对幸福感产生没有太大影响。

```{r}
#列表和分组画图查看各教育程度与各幸福等级的占比
sjPlot::sjt.xtab(data$happiness,data$edu,show.col.prc = TRUE,show.row.prc = TRUE)
sjPlot::sjp.xtab(data$happiness,data$edu,show.total = FALSE,show.prc = FALSE,show.n = FALSE)
```
...
table1 <- sjPlot::sjt.xtab(data$happiness,data$equity,show.col.prc = TRUE,show.row.prc = TRUE) 

```
 -  非常不幸福的人中间认为社会完全不公平的占比最多为45.2%；非常幸福的人中间认为社会比较公平的占比最多为53.5%；从整体上看，认为社会比较公平且比较幸福的人占比最多。因此判断社会公平的建设有助于提高公民幸福感。

**探索个人观点与社会观点的一致性（view）与幸福感（happiness)的关系**

```{r view}
#列表和分组画图查看观点一致性与各幸福等级的占比
sjp.xtab(data$happiness,data$view,show.total = FALSE,show.prc = TRUE,show.n = FALSE)
sjPlot::sjt.xtab(data$happiness,data$view,show.col.prc = TRUE,show.row.prc = TRUE)
```
-  分析可知，比较幸福（happiness=4）并且与社会大众认知基本保持一致（view>=3)的人数占主体。
