...
```{r}
image_path = "images/image_one.png"  
...
```{r}
image_path = "images/image_two.png"  
rmarkdown::html_notebook_output_img(path = image_path, format = "png")
```
...
```{r}
image_path = "images/image_three.png"  
rmarkdown::html_notebook_output_img(path = image_path, format = "png")
```
...
```{r}
library(tidyverse)
data_original<-read.csv("data/happiness_train_abbr.csv")
```
...
```{r}
data_index<-read.csv("data/happiness_index_1.csv")
data_index
```
...
```{r}
library(tidyverse)
data_original<-read.csv("data/happiness_train_clean.csv")
...
```{r}
happy<-data_original[,c(3:38)]
library(psych)
corr.test(happy,use="complete")
```

```{r}
library(corrgram)
corrgram(happy,order=T,lower.panel = panel.shade,upper.panel = panel.shade,text.panel = panel.txt,main="pie")

```

-  相关系数图d额色彩越深，饱和度越高，变量相关性越大，我么可知这36个变量间存在相关性。
...
- 当通过一系列连续型和/或类别型预测变量来预测计数型因变量时，泊松回归是常用的工具,我们的变量中存在大量的类别型变量，故采用泊松回归建立模型。   
...
```{r}
#考虑变量间可能存在的交互影响，生成模型2. 交互影响：即当影响行为的一个因子与另一个因子共同起作用时，它们对该行为产生与各自单独作用时截然不同的影响。例如：A对C有作用，B对C有作用，当A与B联合（A*B）时，对C产生协同或拮抗作用，这种作用称为交互作用。如果A与B对C不产生这作用，则不存在交互作用
...
- 逐步回归由系统自动挑选最佳的回归方程，我们可以看到系统自动去掉了不显著的交互项，级该交互效应在我们所研究的问题上没有统计学上的意义。最终得到的公式为 formula = happiness ~ religion + health + age + equity + inc_ability    
...
```{r}
#选取调查类型(surv_type)、性别(gender)、教育(edu1)、健康(health）、收入(income1)、社会公平(equity)、社会观点一致性(view)、收入合理性(inc_ability)8个自变量进行回归,选用cumalative link model模型进行拟合
...
-  由逐步回归结果可得happiness ~ survey_type + gender + edu1 + income1 + health + equity + view + inc_ability。

### **模型结果可视化**
```{r visualize}
library(ggeffects)

dat1 <- ggpredict(fm2,terms = list("health"))
...
```{r}
dat2 <- ggpredict(fm2,terms = list("gender"))
...
```{r}
dat3 <- ggpredict(fm2,terms = list("income1"))
...
- 可以看出，对于未平衡的数据，正确率约为79%，对于平衡后的数据集，正确率约为67%
- 这主要是由于在非平衡数据中，模型倾向于将绝大部分的数据预测为幸福,而在平衡数据中，尽管正确率降低，但还是高于随机猜测的
...
-  使用nnet预测的成功概率大于随机猜测50%，因此认为神经网络预测效果较好。
